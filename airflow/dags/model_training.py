...
def trained_model(**kwargs):
    ...
    logging.info("Linear Regression Train Score: %f", train_score)
    logging.info("Linear Regression Test Score: %f", test_score)
    logging.info("===============================================")
    ...
    task_instance.xcom_push(key='linear_regression_model', value=json.dumps(model_dict))

# Function to save model to MinIO
def save_model(bucket_name, object_name, **kwargs):
    try:
        # Retrieve the serialized model from XCom
        model_dict_str = kwargs['ti'].xcom_pull(task_ids='train_model_task', key='linear_regression_model')
